				...
				<PopoverTrigger asChild>
					<div
						className={cn(
							...
							isInCart ? 'bg-zinc-900 hover:bg-zinc-900/90' : `${color.bg} ${color.hover}`,
							className
						)}
						ref={ref}
						{...props}
					>
						<span className={cn('text-xs font-medium', isInCart ? 'text-white' : color.text)}>{seat.place}</span>
					</div>
				</PopoverTrigger>
				<PopoverContent className="w-64">
					<div className="flex flex-col gap-1 text-sm">
						<div className="flex justify-between">
							<span className="text-zinc-500">{t('seatmap.row')}</span>
							<span className="font-medium">{seatRow}</span>
						</div>
						<div className="flex justify-between">
							<span className="text-zinc-500">{t('seat.seat')}</span>
